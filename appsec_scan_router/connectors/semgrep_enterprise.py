...
from .utils import mapping, merge_location, repository_location, sequence
...
class SemgrepConnector:
    ...
    def status(self) -> ConnectorStatus:
        ...
        return ConnectorStatus(
            ...
            "Ready"
            if configured
            else "SEMGREP_APP_TOKEN is not configured for Semgrep Enterprise.",
        )
...
CONNECTOR_DEFINITION = ConnectorDefinition(
    key=SemgrepConnector.key,
    name=SemgrepConnector.name,
    connector_type="cloud_api",
    service_managed=True,
    description="Semgrep Enterprise deployment findings and projects API.",
    fields=(
        ConnectorField("endpoint", "API URL"),
        ConnectorField("token", "App token", required=True, secret=True),
    ),
    factory=SemgrepConnector,
)
