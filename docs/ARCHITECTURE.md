...
flowchart LR
  ...
  Core --> Connectors["Scanner Connector Service"]
  Connectors --> ScannerApis["Semgrep / Invicti / NowSecure / SonarQube / OWASP ZAP"]
  Core --> Stores["Apple / Google Store Lookup"]
  Core --> Webhooks["HTTPS Webhooks"]
...
| --- | --- |
| UI service | Login, encrypted user configuration, scan configuration, live logs, report download, database export |
...
| User configuration | Encrypted per-user webhooks, scanner overrides, and remediation timelines |
...
13. Scanner result files return through the ASPM ingestion contract and commit atomically; direct-connector snapshots commit bounded pages, while SARIF profiles remain file imports.
...
The UI writes reports, private scan logs, encrypted run state, encrypted provider credentials, encrypted schedules, and encrypted user integrations under the configured reports/state directory. The Fernet key must remain stable across restarts. A detached worker can outlive a local UI process and is reattached only after its PID and process group are verified. Production deployments should mount durable encrypted storage such as Amazon EFS or Azure Files and store inventory data in managed PostgreSQL.
...
- User webhooks, account-specific scanner settings, and remediation timelines are encrypted and scoped by user; secret values are not returned to the browser.
...
- Outbound webhooks require HTTPS except for loopback destinations and can use bearer authentication, custom headers, and HMAC-SHA256 signatures.
